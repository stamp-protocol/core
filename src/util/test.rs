...
pub(crate) fn create_fake_identity<R: RngCore + CryptoRng>(rng: &mut R, now: Timestamp) -> (SecretKey, Identity, AdminKey) {
...
pub(crate) fn setup_identity_with_subkeys<R: RngCore + CryptoRng>(rng: &mut R) -> (SecretKey, IdentityInstance) {
    let master_key = SecretKey::new_xchacha20poly1305(rng).unwrap();
    ...
    let identity_instance = IdentityInstance::create(IdentityID::random(), vec![admin_key], vec![policy_con], Timestamp::now())
