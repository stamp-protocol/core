...
impl CryptoKeypair {
    ...
    pub fn seal<R: RngCore + CryptoRng>(&self, rng: &mut R, sender_master_key: &SecretKey, sender_keypair: &CryptoKeypair, data: &[u8]) -> Result<CryptoKeypairMessage> {
    ...
    pub fn reencrypt<R: RngCore + CryptoRng>(self, rng: &mut R, previous_master_key: &SecretKey, new_master_key: &SecretKey) -> Result<Self> {
        match self {
            Self::Curve25519XChaCha20Poly1305 { public, secret: Some(private) } => {
                Ok(Self::Curve25519XChaCha20Poly1305 { public, secret: Some(private.reencrypt(rng, previous_master_key, new_master_key)?) })
            }
...
pub(crate) mod tests {
    ...
    fn cryptokeypair_curve25519xchacha20poly1305_enc_dec() {
        ...
        let message = String::from("HI JERRY I'M BUTCH");
        let sealed = recipient_keypair.seal(&mut rng, &sender_master_key, &sender_keypair, message.as_bytes()).unwrap();
